...
function AppSidebar({ activeLabel, isMobileOpen }: AppSidebarProps) {
  ...
  return (
    <aside className={`dashboard-sidebar ${isMobileOpen ? 'mobile-open' : ''}`}>
      <div className="dashboard-logo">
        ...
        <span className="logo-title">EduFlow</span>
      </div>
        ...
        {navGroups.map((group) => (
          <section key={group.title} className="nav-group">
            ...
            {group.items.map((item) => (
              <button
                key={item.label}
                className={item.label === activeLabel ? 'nav-item active' : 'nav-item'}
                type="button"
                onClick={() => {
        ...
        <div className="pro-upgrade-card">
          <div className="pro-card-header">
            ...
            <div className="pro-card-text">
              <h4>Nâng cấp gói Pro</h4>
              <p>Mở khóa nhiều tính năng</p>
            </div>
          </div>
          <button type="button" className="btn-pro-upgrade">
            Nâng cấp ngay ➔
          </button>
        </div>
      </nav>
